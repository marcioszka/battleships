...
def remove_ship(turn_counter: int, 
    ...
    if turn_counter % 2 == 0:
        ...
        for ship_name, ship_position in Globals.PLAYER1_SHIPS.items():
          ships_position.append([ship_position])
          ships_on_board.append(ship_name)
    ship_to_remove = input("You've placed following ships on your board: " + ', '.join(ships_on_board) + ". Which one would you like to remove?")
    while ship_to_remove not in ships_on_board:
        ship_to_remove = input("It's not your ship. Choose again out of: " + ', '.join(ships_on_board) + ".")
    ship_index = 0
    ship_index = ships_on_board.index(ship_to_remove)
    for row, col in ships_position[ship_index]:
        player_board[row][col] = 'O'
    ships_position = ships_position.remove(ship_index)
    ships_on_board = ships_on_board.remove(ship_index)
    Globals.PLAYER2_SHIPS.pop(ship_to_remove) if turn_counter % 2 == 0 else Globals.PLAYER1_SHIPS.pop(ship_to_remove)
    return player_board
...
def check_for_winner(turn_counter: int) -> str: #tuple[bool, int]:
    """Check if a player won after his last move."""
    winner = ""
    if turn_counter % 2 == 0 and len(Globals.PLAYER2_SHIPS) == 0:
        winner = "Player 2"
    elif turn_counter % 2 != 0 and len(Globals.PLAYER1_SHIPS) == 0:
        winner = "Player 1"
    return winner
